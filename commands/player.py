...
class PlayerCommands(commands.Cog):
    ...
    async def start(self, interaction: discord.Interaction, player_name: str, position: str):
        ...
        async with db.pool.acquire() as conn:
            await conn.execute('''
                               INSERT INTO players (user_id, discord_username, player_name, position,
                                                    age, overall_rating, pace, shooting, passing, dribbling, defending,
                                                    physical,
                                                    potential, team_id, league, joined_week, contract_wage,
                                                    contract_years)
        ...
        if assigned_team != 'free_agent':
            async with db.pool.acquire() as conn:
                await conn.execute('''
                                   INSERT INTO transfers (user_id, from_team, to_team, fee, wage, contract_length,
                                                          transfer_type)
                                   VALUES ($1, $2, $3, $4, $5, $6, $7)
                                   ''', interaction.user.id, 'free_agent', assigned_team, 0, wage, contract_years,
                                   'signing')
        ...
        if assigned_team != 'free_agent':
            ...
            # ADD TEAM CREST - FULL DEBUG VERSION
            try:
                from utils.football_data_api import get_team_crest_url
                crest_url = get_team_crest_url(assigned_team)

                # FULL DEBUG LOGGING
                print(f"🔍 Team ID: {assigned_team}")
                print(f"🔍 FULL CREST URL: '{crest_url}'")
                print(f"🔍 URL LENGTH: {len(crest_url) if crest_url else 0}")
                print(f"🔍 URL TYPE: {type(crest_url)}")

                if crest_url:
                    embed.set_thumbnail(url=crest_url)
                    print(f"✅ Set thumbnail successfully")
                else:
                    print(f"❌ No crest URL returned for: {assigned_team}")
    ...
    async def profile(self, interaction: discord.Interaction, user: discord.User = None):
        ...
        if player['team_id'] != 'free_agent':
            ...
            try:
                ...
                # FULL DEBUG LOGGING
                print(f"🔍 PROFILE - Team ID: {player['team_id']}")
                print(f"🔍 PROFILE - FULL URL: '{crest_url}'")
                print(f"🔍 PROFILE - URL LENGTH: {len(crest_url) if crest_url else 0}")
                print(f"🔍 PROFILE - URL TYPE: {type(crest_url)}")

                if crest_url:
                    embed.set_thumbnail(url=crest_url)
                    print(f"✅ PROFILE - Set thumbnail successfully")
                else:
                    print(f"❌ PROFILE - No crest URL returned")
            except Exception as e:
