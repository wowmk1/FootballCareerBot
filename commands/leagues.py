...
async def setup(bot):
    await bot.add_cog(LeagueCommands(bot))            name="🔑 Key",
            value="🟢 Champions League\n🔵 Europa League\n🔴 Relegation",
            inline=False
        )
...
async def setup(bot):
    await bot.add_cog(LeagueCommands(bot))        embed.set_footer(text=f"Season {state['current_season']} • Week {state['current_week']}")
