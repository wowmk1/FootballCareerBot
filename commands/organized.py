...
class OrganizedCommands(commands.Cog):
    ...
    @app_commands.command(name="player", description="👤 Player information and stats")
    ...
    @app_commands.command(name="league", description="🏆 League tables, fixtures, and stats")
    @app_commands.describe(
        action="What to view",
        league="Which league"
    )
    ...
    async def league_cmd(
        ...
        if action == "table":
            await self._show_table(interaction, league)
        elif action == "scorers":
